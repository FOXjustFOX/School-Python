...
while True:
    ...
    print(dct[ct][1] - cf, cs)

    sleep(1)

    if dct[ct][1] - cf < cs:  # if the space between trucks back and car front is less than car speed
...
# car speed changes
    if clane == 0:  # is on right lane
        ...
        if dct[ct][0] <= cf - cl and dct[ct+1][1] > cf:  # if front of the ct truck is less or equal to car back
            ...
            lchanges += 1  # add one lane change
            ct += 1  # go on to the next truck
            clane = 0  # change the lane to right
            print(f"changes {lchanges}")

            clane = 0  # change the lane to left

        if ct == noft and dct[ct][0] <= cf - cl:  # if the last truck is in the left lane and there's place in the
            # overtaken
            lchanges += 1  # add one lane change
            print(f"changes {lchanges}")
